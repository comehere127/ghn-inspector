...
function useReleasedVersions({ currentVersion }: { currentVersion: string }) {
	...
	const fetchReleases = async () => {
		const response = await fetch(
			"https://api.github.com/repos/comehere127/ghn-inspector/releases",
		);
		setReleases({
		...
		case VersionStatus.Loading:
			return (
				<div className="current-version">
					<span>Your version: {packageJSON.version}</span>
			...
			return (
				<div className="current-version">
					...
					<div>
						A{" "}
						<a
							href="https://github.com/comehere127/ghn-inspector/releases"
							target="_blank"
							rel="noreferrer"
						>
							newer version
